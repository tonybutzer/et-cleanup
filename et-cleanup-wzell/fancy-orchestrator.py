...
def return_available_memory():
    ...
    available_memory = 10.5 # hack for now tony
    return float(available_memory)


class Bzipper:
    def __init__(self, yml_file='bzipper.yml'):
        print('Bzipper instaniated', yml_file)
        self.yml_file = yml_file
        self._read_yml()
        self.client = docker.from_env()
    
    def __repr__(self):
        return(json.dumps(self.etm_parms, indent=2))
    ...
    def _event_loop(self, year_to_process, end_year, product):
        ...
        while year_to_process <= end_year:
            ...
            if (mem and cpu and containers):
                ...
                print("skipping starting year", year_to_process)
                year_to_process = year_to_process + 1
            #END While


# ============================================================
        print(f"EVENT LOOP COMPLETE  {product}")
